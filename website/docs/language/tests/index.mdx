...
Extending the previous [example](#example) to allow our tests to specify the region instead of the configuration:
...
> **Warning:** As Terraform executes a destroy operation at the conclusion of each test file you must provide adequate provider definitions at the file level such that Terraform can infer the correct providers to use during the destroy operation.
