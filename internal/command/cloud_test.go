...
func newCloudPluginManifestHTTPTestServer(t *testing.T) *httptest.Server {
	...
	mux.HandleFunc("/0.1.0-prototype/", func(w http.ResponseWriter, r *http.Request) {
		filename := path.Base(r.URL.Path)
		fileToSend, err := os.Open(path.Join(initialPath, fmt.Sprintf("testdata/cloud-archives/%s", filename)))
		if err != nil {
			t.Fatalf("an unknown file was requested: %q", r.URL.Path)
		}
		defer fileToSend.Close()
		io.Copy(w, fileToSend)
	})
